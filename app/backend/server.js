  ...
  const faqsDb = require('./db/faqs');
  const supportTicketsDb = require('./db/supportTickets');
...
const fetch =
  ...
  app.get('/api/tutor/faq', async (req, res) => {
    try {
      if (!ensureDb(res)) {
        return; // ensureDb already responded
      }
      ...
      return res.status(500).json({ error: error.message || 'Failed to fetch FAQ entries' });
    }
  });

  // Delete a FAQ entry by id
  app.delete('/api/tutor/faq/:id', async (req, res) => {
    try {
      if (!ensureDb(res)) {
        return; // ensureDb already responded
      }
      ...
      return res.json({ message: 'FAQ entry deleted successfully' });
    } catch (error) {
      console.error('Error deleting FAQ entry:', error);
      return res.status(500).json({ error: error.message || 'Failed to delete FAQ entry' });
    }
  });
  ...
  app.post('/api/support-tickets', async (req, res) => {
    try {
      ...
      if (!ticketData.creator_id || !ticketData.team_id || !ticketData.subject || !ticketData.description) {
        return res.status(400).json({ error: 'creator_id, team_id, subject, and description are required' });
      }
      const result = await supportTicketsDb.createSupportTicket(ticketData);
      return res.status(201).json(result.rows ? result.rows[0] : result);
    } catch (error) {
  ...
  // Fetch support tickets, optionally filtered by team_id or creator_id
  app.get('/api/support-tickets', async (req, res) => {
    try {
      if (!ensureDb(res)) {
        return; // ensureDb already responded
      }
      const { team_id, creator_id } = req.query;
      const tickets = await supportTicketsDb.getSupportTickets({ team_id, creator_id });
  ...
  app.put('/api/support-tickets/:id', async (req, res) => {
    try {
      ...
      const updated = await supportTicketsDb.updateSupportTicket(id, updateData);
      if (!updated) {
      ...
      return res.status(500).json({ error: error.message || 'Failed to update support ticket' });
    }
  });
  ...
  app.delete('/api/support-tickets/:id', async (req, res) => {
    try {
      ...
      const id = parseInt(req.params.id, 10);
      const deleted = await supportTicketsDb.deleteSupportTicket(id);
      if (!deleted) {
        return res.status(404).json({ error: 'Support ticket not found' });
      }
      ...
      return res.status(500).json({ error: error.message || 'Failed to delete support ticket' });
    }
  });
