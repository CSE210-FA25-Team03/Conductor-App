...
1. Repository Pattern
   ...
   └─ Allows JSON ↔ PostgreSQL swapping
   └─ Used in: TeamsRepository, EvaluationsRepository

2. Factory Pattern
   └─ repositoryFactory.js creates appropriate repository
   └─ Decouples instantiation from usage

3. MVC-Style Separation
   └─ Frontend: HTML (View) + JS (Controller)
   └─ Backend: Routes (Controller) + Repository (Model)

4. Client-Side State Management
   └─ localStorage for session/user data
   └─ In-memory objects (attendanceSessions, evaluationCache)
...
8. Event-Driven UI
   ...
   └─ Forms submit and validate data
   └─ Modals open/close dynamically
```
...
│  DEVELOPMENT (Current)                          │
│  ──────────────────                             │
│  ⚠️  No authentication backend                  │
│  ⚠️  Roles set client-side (localStorage)       │
│  ⚠️  Data persisted in JSON (local files)       │
│  ⚠️  No HTTPS                                   │
│  ✅ ESLint for code quality                    │
│  ✅ Codacy for vulnerability scanning          │
│                                                 │
│  PRODUCTION (Recommended)                       │
│  ──────────────────────                         │
│  ✅ Implement OAuth 2.0 / SAML auth            │
